...
from model2mlir.capture.dynamo_baseline import DynamoReport, collect_diagnostics
from model2mlir.capture.torchao_pipeline import QuantizationConfig, apply_quantization
from model2mlir.capture.unsupported import UnsupportedOpResolution, recover_unsupported_operators
from model2mlir.capture.unsupported.introspect import runtime_versions
from model2mlir.ir.decompositions import DECOMPOSITION_TABLE
from model2mlir.models import CaptureMode
...
def capture_model(
    ...
    # errors during torch.export tracing. Skip the move when already on CPU:
    # .to() on quantized tensor-subclass weights forces a swap_tensors that trips
    # the weakref guard ("Couldn't swap Linear.weight").
    if any(getattr(p, "device", None) is not None and p.device.type != "cpu" for p in model.parameters()):
        model = model.to("cpu")
