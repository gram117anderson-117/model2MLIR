...
def _static_shape(shape_like: Any) -> list[int]:
    ...
    :func:`model2mlir.ir.import_fx._coerce_static_dim`. Symbolic
...
def _attach_region_id(op: Operation, region_id: str) -> None:
    ...
    op.attributes["model2mlir.region_id"] = StringAttr(region_id)
...
def decompose_linear(
    ...
    transpose.attributes["model2mlir.dispatch_id"] = StringAttr(transpose_rid)
    ...
    matmul.attributes["model2mlir.dispatch_id"] = StringAttr(rid)
    ...
    # transpose region.
    matmul.attributes["model2mlir.transposed_b"] = StringAttr("true")
...
def _scalar_to_tensor(scalar: Any, like_type: TensorType) -> tuple[list[Operation], SSAValue]:
    ...
    cast.attributes["model2mlir.cast_to"] = StringAttr(str(elem))
...
def decompose_convolution(operands, meta, node_name):
    ...
    im2col_call.attributes["model2mlir.dispatch_id"] = StringAttr(im2col_rid)
    ...
    w_flat_call.attributes["model2mlir.dispatch_id"] = StringAttr(w_flat_rid)
    ...
    matmul.attributes["model2mlir.dispatch_id"] = StringAttr(mm_rid)
    ...
    reshape_call.attributes["model2mlir.dispatch_id"] = StringAttr(reshape_rid)
...
def _element_type_from_meta(meta: dict[str, Any]) -> Any:
    ...
    if hasattr(torch, "float8_e4m3fn") and d == torch.float8_e4m3fn:
        from model2mlir.ir.types import Float8E4M3FNType

        return Float8E4M3FNType()
    if hasattr(torch, "float8_e5m2") and d == torch.float8_e5m2:
        from model2mlir.ir.types import Float8E5M2Type
...
def decompose_quantize_per_tensor(operands, meta, node_name):
    ...
    """
    from model2mlir.ir.quant.ops import QuantizePerTensorOp
...
def decompose_dequantize_per_tensor(operands, meta, node_name):
    ...
    from model2mlir.ir.quant.ops import DequantizePerTensorOp
...
def decompose_quantize_per_channel(operands, meta, node_name):
    ...
    along ``axis``.
    """
    from model2mlir.ir.quant.ops import QuantizePerChannelOp
...
def decompose_dequantize_per_channel(operands, meta, node_name):
    ...
    from model2mlir.ir.quant.ops import DequantizePerChannelOp
...
def decompose_quantize_per_group(operands, meta, node_name):
    ...
    quant_max, dtype)``.
    """
    from model2mlir.ir.quant.ops import QuantizePerGroupOp
...
def decompose_dequantize_per_group(operands, meta, node_name):
    ...
    from model2mlir.ir.quant.ops import DequantizePerGroupOp
...
def decompose_weight_int8pack_mm(operands, meta, node_name):
    ...
    from model2mlir.ir.quant.ops import WeightInt8PackMMOp
...
def decompose_weight_int4pack_mm(operands, meta, node_name):
    ...
    """
    from model2mlir.ir.quant.ops import WeightInt4PackMMOp
...
def decompose_weight_int4pack_qm(operands, meta, node_name):
    ...
    from model2mlir.ir.quant.ops import WeightInt4PackQMOp
...
def decompose_choose_qparams_per_tensor(operands, meta, node_name):
    ...
    from model2mlir.ir.quant.ops import ChooseQParamsPerTensorOp
...
def decompose_choose_qparams_per_channel(operands, meta, node_name):
    ...
    from model2mlir.ir.quant.ops import ChooseQParamsPerChannelOp
