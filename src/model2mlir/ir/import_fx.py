...
from model2mlir.ir.decompositions import (
    DECOMPOSITION_TABLE,
    DecompFn,
    reset_region_counters,
)
from model2mlir.ir.types import Float8E4M3FNType, Float8E5M2Type
...
def _forward_fx_meta(
    ...
    if effective_hint and "model2mlir._pattern_hint" not in op.attributes:
        op.attributes["model2mlir._pattern_hint"] = StringAttr(str(effective_hint))

    if isinstance(fx_meta, dict):
        if fx_meta.get("_compgen_transpose_absorbed") and "model2mlir.transpose_absorbed" not in op.attributes:
            op.attributes["model2mlir.transpose_absorbed"] = StringAttr("true")
        if fx_meta.get("_compgen_fuse_dequant") and "model2mlir.fuse_dequant" not in op.attributes:
            op.attributes["model2mlir.fuse_dequant"] = StringAttr("true")
...
def _annotate_transposes_and_matmuls(module: ModuleOp) -> None:
    ...
    for op in module.walk():
        if isinstance(op, TransposeOp):
            existing_rid = op.attributes.get("model2mlir.region_id")
            if existing_rid is None:
                rid = f"transpose_{transpose_counter}"
                transpose_counter += 1
                op.attributes["model2mlir.region_id"] = StringAttr(rid)
            else:
                rid = existing_rid.data if isinstance(existing_rid, StringAttr) else None
            if rid and "model2mlir.dispatch_id" not in op.attributes:
                op.attributes["model2mlir.dispatch_id"] = StringAttr(rid)
    ...
    for op in module.walk():
        if not isinstance(op, MatmulOp):
            continue
        if "model2mlir.transposed_b" in op.attributes:
        ...
        try:
            ...
            perm = None
        if perm == [1, 0]:
            op.attributes["model2mlir.transposed_b"] = StringAttr("true")
