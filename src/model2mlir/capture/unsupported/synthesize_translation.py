...
from model2mlir.capture.unsupported.classify import UnsupportedClassification
from model2mlir.capture.unsupported.detect import UnsupportedOperatorIssue
from model2mlir.capture.unsupported.introspect import UnsupportedOpDossier, parse_target
from model2mlir.ir.decompositions import DecompFn, DecompResult
...
def _make_external_call_translation(target: str) -> SynthesizedPayloadTranslation:
    ...
    def translate(
        ...
        call.attributes["model2mlir.region_id"] = StringAttr(f"{region_prefix}_{node_name}")
